...
"""
Nutrition Tools
Advanced tools for nutrition analysis, food database operations, and meal planning.
"""
...
# Remove all imports from configurations.nutrition_config since they are not used in the recipe search tool
...
def extract_nutrients_from_page_basic(url: str, target_nutrient: str = "protein", amount: int = 30) -> Dict[str, Any]:
    """
    Enhanced web scraping that extracts detailed content for meal planning.
...
def extract_structured_content(soup: BeautifulSoup, target_nutrient: str, amount: int) -> Dict[str, Any]:
    """
    Extract structured content including recipes, meal plans, and nutritional information.
    """
    content = {
        "title": "",
        "description": "",
    ...
    try:
        ...
        # Extract recipes (look for recipe-specific structures)
        recipes = []
        recipe_selectors = [
            '.recipe', '.recipe-card', '[itemtype*="Recipe"]',
            '.recipe-content', '.recipe-details', '.meal-recipe'
        ]
        
        for selector in recipe_selectors:
            recipe_elements = soup.select(selector)
            for recipe in recipe_elements[:5]:  # Limit to first 5 recipes
                recipe_text = recipe.get_text().strip()
                if len(recipe_text) > 50:  # Only include substantial content
                    recipes.append(recipe_text[:500])  # Limit length
        ...
        meal_plan = []
        meal_selectors = [
            '.meal-plan', '.daily-plan', '.menu', '.meal-schedule',
            '.breakfast', '.lunch', '.dinner', '.snack'
        ]
        
        for selector in meal_selectors:
            meal_elements = soup.select(selector)
            for meal in meal_elements[:10]:  # Limit to first 10 meals
                meal_text = meal.get_text().strip()
                if len(meal_text) > 30:
                    meal_plan.append(meal_text[:300])
        
        content["meal_plan"] = meal_plan
        
        # Extract ingredients lists
        ingredients = []
        ingredient_selectors = [
            '.ingredients', '.ingredient-list', 'ul li', '.recipe-ingredients'
        ]
        ...
        for selector in ingredient_selectors:
            ...
            for ingredient in ingredient_elements[:20]:  # Limit to first 20 ingredients
                ...
                if len(ingredient_text) > 5 and len(ingredient_text) < 100:
                    # Filter for likely ingredients (contains numbers or common food words)
        ...
        nutritional_info = []
        nutrition_selectors = [
            '.nutrition', '.nutritional-info', '.macros', '.nutrition-facts',
            '.calories', '.protein-content', '.carbs', '.fat'
        ]
        
        for selector in nutrition_selectors:
            nutrition_elements = soup.select(selector)
            for nutrition in nutrition_elements[:10]:
                nutrition_text = nutrition.get_text().strip()
                if len(nutrition_text) > 10:
                    nutritional_info.append(nutrition_text[:150])
        
        content["nutritional_info"] = nutritional_info
        ...
        if content["recipes"]:
            all_content.append(f"Recipes: {len(content['recipes'])} found")
        if content["meal_plan"]:
            all_content.append(f"Meal Plan: {len(content['meal_plan'])} items")
...
def parse_nutrition_query(query: str) -> Optional[Dict[str, Any]]:
...
async def search_and_filter_nutrition(query: str) -> Dict[str, Any]:
    parsed = parse_nutrition_query(query)
    if not parsed:
...
# Register as a LangChain Tool using the new @tool decorator
from langchain_core.tools import tool

@tool
async def nutrition_recipe_search_tool(query: str) -> Dict[str, Any]:
    """
    Searches Google and YouTube for recipes or videos that match a specific nutrient target.
    
    Args:
        query: Natural language query like '30g protein for dinner' or 'high protein recipes'
