...
# ZivoDoc Development Script
# Master script for ZivoDoc iOS app development workflows

set -e  # Exit on any error

echo "🚀 ZivoDoc Development Script"
echo "============================="
...
show_usage() {
    ...
    echo "  $0 dev                           # Update and build ZivoDoc"
...
if [ ! -f "project.yml" ]; then
    print_error "project.yml not found. Please run this script from the ZivoDoc project root."
    exit 1
fi
...
case $COMMAND in
    setup)
        print_header "🔧 Initial Project Setup for $PROJECT_NAME"
        ...
        print_status "1. Open ${PROJECT_NAME}.xcodeproj in Xcode"
        print_status "2. Set your development team in project settings"
        print_status "3. Run: $0 dev"
        ;;
        
    update)
        print_header "🔄 Updating Xcode Project for $PROJECT_NAME"
        if [ -f "scripts/update_project.sh" ]; then
            bash scripts/update_project.sh
        else
            xcodegen generate
    ...
    build)
        print_header "🔨 Building Debug for $PROJECT_NAME"
        if [ -f "scripts/build.sh" ]; then
            bash scripts/build.sh "$@"
        else
            print_error "build.sh not found in scripts/"
            exit 1
        fi
        ;;
        
    release)
        print_header "🔨 Building Release for $PROJECT_NAME"
        if [ -f "scripts/build.sh" ]; then
    ...
    archive)
        print_header "📦 Creating Archive for $PROJECT_NAME"
        if [ -f "scripts/build.sh" ]; then
    ...
    clean)
        print_header "🧹 Cleaning Build Artifacts for $PROJECT_NAME"
    ...
    lint)
        print_header "🔍 Running SwiftLint for $PROJECT_NAME"
        if command -v swiftlint &> /dev/null; then
    ...
    dev)
        print_header "🚀 Full Development Workflow for $PROJECT_NAME"
