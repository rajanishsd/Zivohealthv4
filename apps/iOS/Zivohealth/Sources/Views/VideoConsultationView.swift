...
class VideoConsultationViewModel: ObservableObject {
    ...
    private func checkLiveVideoAvailability() async -> Bool {
        do {
            let url = try await networkService.getLiveKitURL()
            guard !url.isEmpty else {
                await MainActor.run { self.error = "Live video is unavailable right now (no URL)." }
                return false
            }
            // Try generating a short-lived token to verify server-side config
            let identity = "healthcheck-\(UUID().uuidString.prefix(8))"
            _ = try await networkService.createLiveKitToken(
                room: "healthcheck",
            ...
            await MainActor.run { self.error = "Live video is currently unavailable. Please try again later. (\(error.localizedDescription))" }
            return false
        }
    }
    ...
    func requestVideoCall(with doctor: Doctor) {
        ...
        Task {
            let healthy = await checkLiveVideoAvailability()
            guard healthy else { return }
            await MainActor.run {
                self.selectedDoctor = doctor
                self.requestType = .individual
                self.showingCallRequest = true
            }
