...
struct UploadPrescriptionView: View {
    ...
    @State private var chatPlaceholderMessage = "update my health record with prescription from the uploaded file. dont ask any question back"
    @State private var submittedImageThumb: UIImage?
    @AppStorage("uploadPrescription_isAnalyzing") private var persistedAnalyzing: Bool = false
    @AppStorage("uploadPrescription_hasResponse") private var persistedHasResponse: Bool = false
    @AppStorage("uploadPrescription_image_path") private var persistedImagePath: String = ""
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text("Upload a file or take a photo to automatically update your medications")
                            .font(.subheadline)
                    ...
                    if let previewImage = selectedImage {
                        VStack(alignment: .leading, spacing: 12) {
                            Text("Selected Prescription (Photo)")
                                .font(.headline)
                            ZStack {
                    ...
                    if let docName = selectedDocumentName {
                        VStack(alignment: .leading, spacing: 12) {
                            Text("Selected Prescription (File)")
                                .font(.headline)
                            HStack(spacing: 12) {
        ...
        .modifier(
            ConditionalPrescriptionPhotosPickerModifier(
        ...
        .sheet(isPresented: $showingCameraPicker) {
            PrescriptionImagePicker(sourceType: .camera) { image in
                selectedImage = image
                selectedImageData = image.jpegData(compressionQuality: 0.8)
            }
        }
        .sheet(isPresented: $showingLegacyImagePicker) {
            PrescriptionImagePicker(sourceType: legacySourceType) { image in
                selectedImage = image
                selectedImageData = image.jpegData(compressionQuality: 0.8)
            }
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if !(entryMode == .photo && didSubmitToChat) {
                HStack(alignment: .top, spacing: 8) {
                    Circle().fill(Color.green).frame(width: 24, height: 24).overlay(Text("Y").font(.caption).foregroundColor(.white))
                    Text(didSubmitToChat ? (lastSubmittedDisplayText.isEmpty ? "" : lastSubmittedDisplayText) : "Update health records with uploaded prescription")
                        .font(.subheadline)
                        .foregroundColor(.primary)
                        .padding(10)
                        ...
                        HStack(alignment: .bottom, spacing: 8) {
                            ...
                            Button(action: {
                                ...
                                Image(systemName: "arrow.up.circle.fill")
                                    .font(.system(size: 32))
                                    .foregroundColor(followUpMessage.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? .gray : .green)
                            }
    ...
    private func handleGlobalSubmit() {
        ...
        case .photo:
            ...
            let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("prescription_\(UUID().uuidString).jpg")
            try? data.write(to: tempURL)
            submittedImageThumb = previewImage
            lastSubmittedDisplayText = "Update health records with uploaded prescription"
            didSubmitToChat = true
        ...
        case .document:
            ...
            lastSubmittedDisplayText = "Update health records with uploaded prescription file"
            didSubmitToChat = true
            persistedAnalyzing = true
            Task {
                await chatViewModel.createNewSession()
    ...
    private func handleDocumentSelection(url: URL) {
        ...
        coordinator.coordinate(readingItemAt: url, options: [.forUploading], error: &error) { (fileURL) in
            do {
                ...
                let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("prescription_\(UUID().uuidString).\(fileURL.pathExtension)")
    ...
    private func persistImageDataToTemp(_ data: Data) -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("uploadprescription_current.jpg")
...
// MARK: - UI Helpers
extension UploadPrescriptionView {
    @ViewBuilder
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    .fill(selected ? Color.green : Color(UIColor.systemBackground))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 16)
                    .stroke(selected ? Color.clear : Color.gray.opacity(0.15), lineWidth: 1)
            )
            .shadow(color: selected ? Color.green.opacity(0.25) : Color.black.opacity(0.05), radius: 6, x: 0, y: 3)
        }
        .buttonStyle(.plain)
    }
}
...
struct ConditionalPrescriptionPhotosPickerModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if #available(iOS 16.0, *) {
            content.modifier(PrescriptionPhotosPickerModifier(
...
@available(iOS 16.0, *)
struct PrescriptionPhotosPickerModifier: ViewModifier {
    @Binding var isPresented: Bool
    @Binding var selectedImageData: Data?
    @Binding var selectedImage: UIImage?
