...
public class NetworkService: ObservableObject {
    ...
    // Status WebSocket management (one active per session)
    private var statusSockets: [Int: URLSessionWebSocketTask] = [:]
    private var statusReconnectAttempts: [Int: Int] = [:]
    ...
    private func setupLifecycleObservers() {
        foregroundObserver = NotificationCenter.default.addObserver(
            ...
            Task {
                await self.refreshOnForegroundIfNeeded()
            }
        }
    ...
    private func refreshOnForegroundIfNeeded() async {
        // If token is expired or about to expire within 2 minutes, refresh proactively
        let expiringSoon = isTokenExpiringSoon(window: 120)
        if isTokenExpired() || expiringSoon {
            _ = await tryRefreshAccessTokenIfNeeded()
        }
    }

    private func isTokenExpiringSoon(window: TimeInterval) -> Bool {
        if let expiry = inMemoryTokenExpiry {
            return Date().addingTimeInterval(window) >= expiry
        }
        ...
        if let tokenData = UserDefaults.standard.data(forKey: key),
           let tokenInfo = try? decoder.decode(TokenInfo.self, from: tokenData) {
            return Date().addingTimeInterval(window) >= tokenInfo.expiresAt
        }
        // If we do not have metadata, be conservative and attempt refresh only if we actually have a token
    ...
    private lazy var decoder: JSONDecoder = {
        ...
        decoder.dateDecodingStrategy = .custom { decoder in
            ...
            // Format without timezone: "2025-06-17T16:00:00" (use device timezone)
            ...
            // Format with milliseconds but no timezone: "2025-06-17T16:00:00.000" (use device timezone)
    ...
    private init() {
        ...
        // Configure lifecycle observers so we refresh tokens when the app returns to foreground
        ...
        return AsyncStream { continuation in
            Task {
                do {
                    ...
                    if !sawCompleteOrError {
                        print("⚠️ [NetworkService] SSE ended without complete/error - applying client fallback")
                        // Yield a synthetic completion so callers can clear UI state
                        let synthetic = StreamingChunk(type: "complete", content: nil, progress: 1.0)
                            ...
                            while !shouldStop {
                                ...
                                case .string(let text):
                                    if let data = text.data(using: .utf8) {
                                        do {
                                            let statusMessage = try self.decoder.decode(ChatStatusMessage.self, from: data)
