...
show_usage() {
    ...
    echo "  -s, --scheme SCHEME     Build scheme (default: ZivoHealth)"
...
if [ ! -f "project.yml" ] && [ ! -f "ZivoHealth.xcodeproj/project.pbxproj" ]; then
    print_error "No project configuration found. Please run this script from the Zivohealth project root."
    exit 1
fi
...
if [ "$ARCHIVE" = true ]; then
    ...
    ARCHIVE_PATH="./build/ZivoHealth_$(date +%Y%m%d_%H%M%S).xcarchive"
