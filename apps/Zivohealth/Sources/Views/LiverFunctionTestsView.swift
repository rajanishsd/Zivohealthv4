...
struct LiverFunctionTestsView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var lftTests: [LFTTest] = []
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                // Header Section
                LFTHeader(tests: lftTests)
                
                if isLoading {
                    ProgressView("Loading liver function tests data...")
                        .padding()
                } else if let error = errorMessage {
                    Text("Error loading data: \(error)")
                        .foregroundColor(.red)
                        .padding()
                } else if lftTests.isEmpty {
                    VStack(spacing: 12) {
                        Image(systemName: "leaf.fill")
                            .font(.title)
                            .foregroundColor(.brown)
                        
                        Text("No LFT Data Available")
                            .font(.headline)
                            .fontWeight(.semibold)
                        
                        Text("Upload your lab reports to see liver function test results")
                            .font(.subheadline)
                    ...
                    LazyVStack(spacing: 12) {
                        ForEach(lftTests) { test in
                            LFTTestCard(test: test)
                        }
                    }
                    .padding(.horizontal)
                }
        ...
        .onAppear {
            print("🍃 [LiverFunctionTestsView] onAppear - Loading LFT data")
            loadLFTData()
        }
        .onDisappear {
            print("🍃 [LiverFunctionTestsView] onDisappear")
        }
    }
    
    private func loadLFTData() {
        print("🔄 [LiverFunctionTestsView] Loading LFT data from API...")
        isLoading = true
        errorMessage = nil
        
        LabReportsAPIService.shared.getCategoryTestsData(category: "Liver Function Tests (LFT)")
            .sink(
                receiveCompletion: { completion in
                    switch completion {
                    case .finished:
                        print("✅ [LiverFunctionTestsView] LFT data loading completed")
                    case .failure(let error):
                        print("❌ [LiverFunctionTestsView] Error loading LFT data: \(error)")
                ...
                receiveValue: { response in
                    print("📊 [LiverFunctionTestsView] Received \(response.tests.count) LFT tests")
                    self.convertToLFTModel(response)
                    self.isLoading = false
                }
            )
    ...
    private func convertToLFTModel(_ response: CategoryTestsResponse) {
        lftTests = response.tests.map { testData in
            LFTTest(
                name: testData.name,
...
struct LFTHeader: View {
    ...
    private func generateInsight() -> String {
        ...
        if hasCritical {
            ...
            return "Some liver function markers are elevated. Consider discussing with your doctor."
        } else {
            return "All liver function markers are within normal ranges. Your liver is functioning well."
        }
    }
