...
# Run password reset database migrations (032 and 033) on AWS RDS using Docker
# Usage:
#   run_password_reset_migrations_docker.sh [-p AWS_PROFILE] [-r AWS_REGION] [-i INSTANCE_ID]
...
print_usage() {
  echo "Run password reset database migrations (032 and 033) on AWS RDS using Docker"
  echo "Options:"
...
# Execute Docker migrations step by step to avoid parameter parsing issues
echo "🚀 Running password reset migrations using Docker with proper environment setup..."
...
aws ssm send-command \
  ...
  --instance-ids "$INSTANCE_ID" \
  --document-name "AWS-RunShellScript" \
  --parameters 'commands=["sudo docker exec zivohealth-api-1 bash -c \"cd /app && export PYTHONPATH=/app && alembic current\""]' \
  --output table

echo "⏳ Waiting 10 seconds..."
sleep 10

# Step 3: Run migration 032
echo "🚀 Step 3: Running migration 032 (Add password reset tokens table)..."
aws ssm send-command \
  --profile "$AWS_PROFILE" \
  --region "$AWS_REGION" \
  --instance-ids "$INSTANCE_ID" \
  --document-name "AWS-RunShellScript" \
  --parameters 'commands=["sudo docker exec zivohealth-api-1 bash -c \"cd /app && export PYTHONPATH=/app && alembic upgrade 032_add_password_reset\""]' \
  --output table

echo "⏳ Waiting 15 seconds..."
sleep 15

# Step 4: Run migration 033
echo "🚀 Step 4: Running migration 033 (Update password reset tokens to support doctors)..."
aws ssm send-command \
  --profile "$AWS_PROFILE" \
  --region "$AWS_REGION" \
  --instance-ids "$INSTANCE_ID" \
  --document-name "AWS-RunShellScript" \
  --parameters 'commands=["sudo docker exec zivohealth-api-1 bash -c \"cd /app && export PYTHONPATH=/app && alembic upgrade 033_password_reset_doctors\""]' \
  --output table
...
echo "📊 Step 5: Verifying final migration status..."
aws ssm send-command \
  --profile "$AWS_PROFILE" \
  --region "$AWS_REGION" \
  --instance-ids "$INSTANCE_ID" \
